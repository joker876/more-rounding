 ...
 */function approximate(number,precision=1,unit,locale,unitMap,useSpace=true){var _a;if(!unit){const units=Object.keys(exports.APPROXIMATION_UNITS);for(let i=0;i<units.length;i++){let unit=units[i];let divider=exports.APPROXIMATION_UNITS[unit];let dividedNumber=number/divider;if(dividedNumber>=1&&dividedNumber<1e3){if(locale)return roundToPrecision(dividedNumber,precision).toLocaleString(locale)+(useSpace?" ":"")+unit;return String(roundToPrecision(dividedNumber,precision))+(useSpace?" ":"")+unit}}return String(roundToPrecision(number,precision).toLocaleString(locale))}const divider=exports.APPROXIMATION_UNITS[unit];const unitString=(_a=unitMap===null||unitMap===void 0?void 0:unitMap[unit])!==null&&_a!==void 0?_a:unit;if(locale)return roundToPrecision(number/divider,precision).toLocaleString(locale)+(useSpace?" ":"")+unitString;return String(roundToPrecision(number/divider,precision))+(useSpace?" ":"")+unitString}exports.approximate=approximate;